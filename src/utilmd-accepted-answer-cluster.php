<?php

return [
    'E_0007' => [
        'A06' => 'Zeitreihe akzeptiert',
    ],
    'E_0020' => [
        'A12' => 'Aktivierung durchgeführ',
    ],
    'E_0021' => [
        'A12' => 'Aktivierung durchgeführ',
    ],
    'E_0400' => [
        'A06' => 'Vertrag wurde bereits zum angefragten Kündigungstermin gekündigt',
        'A11' => 'Zustimmung',
    ],
    'E_0401' => [
        'A06' => 'Bestätigung der Abmeldung',
    ],
    'E_0403' => [
        'A31' => 'Zustimmung der Abmeldeanfrage zum in der bereits versendeten Abmeldung (welche noch nicht beantwortet wurde) genannten Termin',
        'A34' => 'Der LFA beendet die Belieferung und teilt sein Lieferendedatum in der Antwort mit',
        'A36' => 'Vertragsverhältnis wurde zum angefragten oder davor liegenden Termin beendet',
        'A38' => 'Ersatzversorgung wurde zum angefragten Termin beendet',
    ],
    'E_0404' => [
        'A51' => 'Zustimmung',
    ],
    'E_0439' => [
        'A06' => 'Zustimmung ohne Korrekturen',
    ],
    'E_0440' => [
        'A01' => 'Gewünschte Gerätekonfiguration liegt zum Termin bereits vor',
    ],
    'E_0455' => [
        'A15' => 'Stammdaten wurden widerspruchsfrei übernommen',
    ],
    'S_0001' => [
        'E15' => 'Zustimmung ohne Korrekturen',
        'Z01' => 'Zustimmung mit Terminänderung',
    ],
    'S_0003' => [
        'E15' => 'Zustimmung ohne Korrekturen',
        'Z43' => 'Zustimmung mit Korrektur von bilanzierungsrel. Daten',
        'Z44' => 'Zustimmung mit Korrektur von nicht bilanzierungsrel. Daten',
    ],
    'E_0453' => [
        'A13' => 'Stammdaten wurden widerspruchsfrei übernommen.'
    ],
    'E_0456' => [
        'A05' => 'Zustimmung',
    ],
    'E_0502' => [
        'A10' => 'Zustimmung',
    ],
    'S_0005' => [
        'E15' => 'Zustimmung ohne Korrekturen',
    ],
    'S_0007' => [
        'E15' => 'Zustimmung ohne Korrekturen',
    ],
    'S_0009' => [
        'E15' => 'Zustimmung ohne Korrekturen',
    ],
    'S_0013' => [
        'E15' => 'Zustimmung ohne Korrekturen',
    ],
    'S_0006' => [
        'E15' => 'Zustimmung ohne Korrekturen',
    ],
    'S_0008' => [
        'E15' => 'Zustimmung ohne Korrekturen',
    ],
    'S_0010' => [
        'E15' => 'Zustimmung ohne Korrekturen',
    ],
    'S_0014' => [
        'E15' => 'Zustimmung ohne Korrekturen',
    ],
    'S_0015' => [
        'E15' => 'Zustimmung ohne Korrekturen',
    ],
    'S_0017' => [
        'E15' => 'Zustimmung ohne Korrekturen',
    ],
    'S_0016' => [
        'E15' => 'Zustimmung ohne Korrekturen',
    ],
    'S_0018' => [
        'E15' => 'Zustimmung ohne Korrekturen',
    ],
    'S_0019' => [
        'E15' => 'Zustimmung ohne Korrekturen',
    ],
    'S_0020' => [
        'E15' => 'Zustimmung ohne Korrekturen',
    ],
    'S_0021' => [
        'ZG2' => 'Gültiges Ergebnis nach der Datenprüfung',
    ],
    'S_0022' => [
        'ZG2' => 'Gültiges Ergebnis nach der Datenprüfung',
    ],
    'S_0023' => [
        'ZG2' => 'Gültiges Ergebnis nach der Datenprüfung',
    ],
    'S_0025' => [
        'ZG2' => 'Gültiges Ergebnis nach der Datenprüfung',
    ],
    'S_0026' => [
        'ZG2' => 'Gültiges Ergebnis nach der Datenprüfung',
    ],
    'S_0028' => [
        'ZG2' => 'Gültiges Ergebnis nach der Datenprüfung',
    ],
    'S_0029' => [
        'ZG2' => 'Gültiges Ergebnis nach der Datenprüfung',
    ],
    'S_0031' => [
        'ZG2' => 'Gültiges Ergebnis nach der Datenprüfung',
    ],
    'S_0032' => [
        'ZG2' => 'Gültiges Ergebnis nach der Datenprüfung',
    ],
    'S_0034' => [
        'ZG2' => 'Gültiges Ergebnis nach der Datenprüfung',
    ],
    'S_0036' => [
        'ZG2' => 'Gültiges Ergebnis nach der Datenprüfung',
    ],
    'S_0038' => [
        'ZG2' => 'Gültiges Ergebnis nach der Datenprüfung',
    ],
    'S_0039' => [
        'E15' => 'Zustimmung ohne Korrekturen',
    ],
    'S_0040' => [
        'E15' => 'Zustimmung ohne Korrekturen',
    ],
    'S_0041' => [
        'ZG2' => 'Gültiges Ergebnis nach der Datenprüfung',
    ],
    'S_0086' => [
        'E15' => 'Zustimmung ohne Korrekturen',
    ],
    'S_0099' => [
        'ZG2' => 'Gültiges Ergebnis nach der Datenprüfung',
    ],
    'S_0082' => [
        'E15' => 'Zustimmung ohne Korrekturen',
    ],
    'S_0094' => [
        'ZG2' => 'Gültiges Ergebnis nach der Datenprüfung',
    ],
    'S_0101' => [
        'ZG2' => 'Gültiges Ergebnis nach der Datenprüfung',
    ],
    'G_0002' => [
        'E15' => 'Zustimmung ohne Korrekturen',
    ],
    'G_0004' => [
        'E15' => 'Zustimmung ohne Korrekturen',
    ],
    'G_0006' => [
        'E15' => 'Zustimmung ohne Korrekturen',
        'Z01' => 'Zustimmung mit Terminänderung',
        'Z44' => 'Zustimmung mit Korrektur von nicht bilanzierungsrel. Daten',
    ],
    'G_0008' => [
        'E15' => 'Zustimmung ohne Korrekturen',
        'Z01' => 'Zustimmung mit Terminänderung',
    ],
    'G_0010' => [
        'E15' => 'Zustimmung ohne Korrekturen',
        'Z01' => 'Zustimmung mit Terminänderung',
    ],
    'G_0012' => [
        'E15' => 'Zustimmung ohne Korrekturen',
        'Z01' => 'Zustimmung mit Terminänderung',
        'Z43' => 'Zustimmung mit Korrektur von bilanzierungsrel. Daten',
        'Z44' => 'Zustimmung mit Korrektur von nicht bilanzierungsrel. Daten',
    ],
    'G_0013' => [
        'E15' => 'Zustimmung ohne Korrekturen',
        'Z43' => 'Zustimmung mit Korrektur von bilanzierungsrel. Daten',
        'Z44' => 'Zustimmung mit Korrektur von nicht bilanzierungsrel. Daten',
    ],
    'G_0015' => [
        'Z15' => 'Ablehnung keine Berechtigung',
    ],
    'G_0016' => [
        'E15' => 'Zustimmung ohne Korrekturen',
    ],
    'G_0017' => [
        'E15' => 'Zustimmung ohne Korrekturen',
    ],
    'G_0018' => [
        'E15' => 'Zustimmung ohne Korrekturen',
    ],
    'G_0019' => [
        'E15' => 'Zustimmung ohne Korrekturen',
    ],
    'G_0022' => [
        'E15' => 'Zustimmung ohne Korrekturen',
    ],
    'G_0023' => [
        'E15' => 'Zustimmung ohne Korrekturen',
    ],
    'G_0024' => [
        'E15' => 'Zustimmung ohne Korrekturen',
    ],
    'G_0025' => [
        'E15' => 'Zustimmung ohne Korrekturen',
    ],
    'G_0026' => [
        'E15' => 'Zustimmung ohne Korrekturen',
    ],
    'G_0027' => [
        'E15' => 'Zustimmung ohne Korrekturen',
    ],
    'G_0028' => [
        'E15' => 'Zustimmung ohne Korrekturen',
    ],
    'G_0029' => [
        'E15' => 'Zustimmung ohne Korrekturen',
    ],
    'G_0030' => [
        'E15' => 'Zustimmung ohne Korrekturen',
    ],
    'G_0031' => [
        'ZG2' => 'Gültiges Ergebnis nach der Datenprüfung',
    ],
    'G_0032' => [
        'ZG2' => 'Gültiges Ergebnis nach der Datenprüfung',
    ],
    'G_0034' => [
        'ZG2' => 'Gültiges Ergebnis nach der Datenprüfung',
    ],
    'G_0035' => [
        'ZG2' => 'Gültiges Ergebnis nach der Datenprüfung',
    ],
    'G_0038' => [
        'ZG2' => 'Gültiges Ergebnis nach der Datenprüfung',
    ],
    'G_0040' => [
        'ZG2' => 'Gültiges Ergebnis nach der Datenprüfung',
    ],
    'G_0041' => [
        'ZG2' => 'Gültiges Ergebnis nach der Datenprüfung',
    ],
    'G_0043' => [
        'ZG2' => 'Gültiges Ergebnis nach der Datenprüfung',
    ],
    'G_0044' => [
        'ZG2' => 'Gültiges Ergebnis nach der Datenprüfung',
    ],
    'G_0045' => [
        'ZG2' => 'Gültiges Ergebnis nach der Datenprüfung',
    ],
    'G_0047' => [
        'ZG2' => 'Gültiges Ergebnis nach der Datenprüfung',
    ],
    'G_0067' => [
        'E15' => 'Zustimmung ohne Korrekturen',
    ],
    'G_0069' => [
        'ZG2' => 'Gültiges Ergebnis nach der Datenprüfung',
    ],
    'G_0076' => [
        'Z23' => 'Messung gescheitert (kein Zugang)',
        'Z24' => 'Messung gescheitert (Kommunikationsstörung)',
        'Z25' => 'Messung gescheitert (Netzausfall)',
        'Z26' => 'Messung gescheitert (Spannungsausfall)',
        'Z27' => 'Messung gescheitert (Gerätewechsel)',
        'Z28' => 'Messung gescheitert (Kalibrierung)',
        'Z29' => 'Messung gescheitert (Gerät arbeitet außerhalb der Betriebsbedingungen)',
        'Z30' => 'Messung gescheitert (Messeinrichtung gestört/defekt)',
        'Z31' => 'Messung gescheitert (Unsicherheit Messung)',
        'ZD7' => 'In der Messlokation ist kein Gerät vorhanden',
        'ZD8' => 'Messlokation real nicht auffindbar',
    ],
    'G_0077' => [
        'Z74' => 'kein Zugang',
        'Z75' => 'Kommunikationsstörung',
        'Z76' => 'Netzausfall',
        'Z78' => 'Gerätewechsel',
        'Z79' => 'Kalibrierung',
        'Z80' => 'Gerät arbeitet außerhalb der Betriebsbedingungen',
        'Z81' => 'Messeinrichtung gestört/defekt',
        'Z82' => 'Unsicherheit Messung',
        'ZD7' => 'In der Messlokation ist kein Gerät vorhanden',
        'ZD8' => 'Messlokation real nicht auffindbar',
    ],
];
